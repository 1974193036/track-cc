...
export class Report {
  ...
  send(data: IEventParams | IEventParams[], beforeSend?: Callback) {
    // 参数归一化
  ...
  fetchReport(url: string, data: IReportParams[]) {
    const requestFun = () => {
      fetch(`${url}`, {
  ...
  xhrReport(url: string, data: IReportParams[]) {
    const requestFun = () => {
  ...
  httpReport(url: string, data: IReportParams[]) {
    if (isSupportFetch()) {
